...
export const ProgramsSection: React.FC<ProgramsSectionProps> = ({
  ...
  /*
   * Filter programs directly from lib/data.ts
   */
  ...
  /*
   * Choose icon based on program/faculty name
   */
  ...
  /*
   * Program description generated only from existing data
   */
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mb-12 max-w-3xl space-y-3 text-center">
          ...
          <p className="text-sm leading-relaxed text-slate-600 sm:text-base">
            Explore undergraduate and postgraduate programs available across
            the university&apos;s academic faculties.
          </p>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          ...
          {filteredPrograms.map((program, index) => (
            <div
              ...
              className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl"
            >
              ...
              <div className="flex items-center justify-between gap-2 border-t border-slate-100 bg-slate-50 p-4">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg bg-[#0D0357] px-3.5 py-2 text-xs font-bold text-amber-300 shadow-sm transition-all hover:bg-indigo-950"
                >
                  <span>Apply Now</span>
                </button>
...
function ApplicationModal({
  ...
  const validateStep = (currentStep: number) => {
    ...
    const invalidField = Array.from(fields ?? []).find(
      (field) =>
        ...
        {submitted ? (
          ...
          <form ref={formRef} onSubmit={submitApplication} noValidate>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`hidden h-0.5 flex-1 sm:block ${
                            item.number < step ? "bg-[#0D0357]" : "bg-slate-200"
                          }`}
                        />
                      </div>
                      <p
            ...
            <div className="px-5 py-6 sm:px-8 sm:py-7">
              {step === 1 && (
                <div className="space-y-5">
                  <div>
                    <h3 className="text-lg font-extrabold text-slate-900">Personal Info</h3>
                    <p className="mt-1 text-sm text-slate-500">Use your official contact details.</p>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className={labelClassName}>
                      Full name <span className="text-rose-500">*</span>
                      <input data-step="1" required name="fullName" autoComplete="name" className={fieldClassName} placeholder="Your full name" />
                    </label>
                    <label className={labelClassName}>
                      Email address <span className="text-rose-500">*</span>
                      <input data-step="1" required type="email" name="email" autoComplete="email" className={fieldClassName} placeholder="name@example.com" />
                    </label>
                    <label className={labelClassName}>
                      Mobile number <span className="text-rose-500">*</span>
                      <input data-step="1" required type="tel" name="phone" autoComplete="tel" className={fieldClassName} placeholder="01XXXXXXXXX" />
                    </label>
                    <label className={labelClassName}>
                      Date of birth <span className="text-rose-500">*</span>
                      <input data-step="1" required type="date" name="dateOfBirth" className={fieldClassName} />
                    </label>
                  </div>
                  <label className={labelClassName}>
                    Present address <span className="text-rose-500">*</span>
                    <textarea data-step="1" required name="address" rows={3} className={fieldClassName} placeholder="House, road, area, district" />
                  </label>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-5">
                  <div>
                    <h3 className="text-lg font-extrabold text-slate-900">Academic Grades</h3>
                    <p className="mt-1 text-sm text-slate-500">Enter the results from your most recent qualifications.</p>
                  </div>
                  <div className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
                    <p className="text-sm font-extrabold text-[#0D0357]">Secondary School Certificate (SSC/O-Level)</p>
                    <div className="mt-3 grid gap-4 sm:grid-cols-2">
                      <label className={labelClassName}>Board <span className="text-rose-500">*</span><input data-step="2" required name="sscBoard" className={fieldClassName} placeholder="e.g. Dhaka" /></label>
                      <label className={labelClassName}>Passing year <span className="text-rose-500">*</span><input data-step="2" required name="sscYear" type="number" min="1990" max="2100" className={fieldClassName} placeholder="2023" /></label>
                      <label className={labelClassName}>GPA / grade <span className="text-rose-500">*</span><input data-step="2" required name="sscGpa" type="number" min="0" max="5" step="0.01" className={fieldClassName} placeholder="5.00" /></label>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
                    <p className="text-sm font-extrabold text-[#0D0357]">Higher Secondary Certificate (HSC/A-Level)</p>
                    <div className="mt-3 grid gap-4 sm:grid-cols-2">
                      <label className={labelClassName}>Board <span className="text-rose-500">*</span><input data-step="2" required name="hscBoard" className={fieldClassName} placeholder="e.g. Dhaka" /></label>
                      <label className={labelClassName}>Passing year <span className="text-rose-500">*</span><input data-step="2" required name="hscYear" type="number" min="1990" max="2100" className={fieldClassName} placeholder="2025" /></label>
                      <label className={labelClassName}>GPA / grade <span className="text-rose-500">*</span><input data-step="2" required name="hscGpa" type="number" min="0" max="5" step="0.01" className={fieldClassName} placeholder="5.00" /></label>
                    </div>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div>
                    <h3 className="text-lg font-extrabold text-slate-900">Program & Submission</h3>
                    <p className="mt-1 text-sm text-slate-500">Review your choice, then submit the application.</p>
                  </div>
                  <label className={labelClassName}>
                    Program you are applying for <span className="text-rose-500">*</span>
                    <select data-step="3" required name="program" defaultValue={initialProgram} className={fieldClassName}>
                      <option value="" disabled>Select a program</option>
                      {PROGRAMS.map((program) => <option key={program.title} value={program.title}>{program.title}</option>)}
                    </select>
                  </label>
                  <label className={labelClassName}>
                    Preferred intake <span className="text-rose-500">*</span>
                    <select data-step="3" required name="intake" defaultValue="" className={fieldClassName}>
                  ...
                  <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-relaxed text-slate-600">
                    <input data-step="3" required name="declaration" type="checkbox" className="mt-0.5 h-4 w-4 accent-[#0D0357]" />
                    <span>I confirm that the information provided is accurate and I agree to the admissions terms. <span className="text-rose-500">*</span></span>
                  </label>
                </div>
              )}

              {error && <p className="mt-5 rounded-xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700" role="alert">{error}</p>}
            </div>
            ...
            <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-slate-200 bg-white px-5 py-4 sm:px-8">
              {step > 1 ? (
                <button type="button" onClick={() => { setError(""); setStep((current) => current - 1); }} className="inline-flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-bold text-[#0D0357] transition hover:bg-indigo-50">
                  <ChevronLeft className="h-4 w-4" /> Back
                </button>
              ) : <span />}
              {step < 3 ? (
                <button type="button" onClick={nextStep} className="inline-flex items-center gap-1.5 rounded-xl bg-[#0D0357] px-5 py-2.5 text-sm font-bold text-amber-300 shadow-sm transition hover:bg-indigo-950">
                  Next step <ChevronRight className="h-4 w-4" />
                </button>
              ) : (
                <button type="submit" className="inline-flex items-center gap-1.5 rounded-xl bg-[#0D0357] px-5 py-2.5 text-sm font-bold text-amber-300 shadow-sm transition hover:bg-indigo-950">
