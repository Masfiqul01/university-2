...
export interface SpecularButtonProps {
  ...
  /** When set the button renders as a link (next/link for internal hrefs). */
  href?: string;
  target?: string;
  rel?: string;
  "aria-label"?: string;
...
const SpecularButton = ({
  ...
  // WebGL contexts are capped per page (~16 in Chrome). Only spin one up while
  // the button is anywhere near the viewport, and tear it down when it leaves,
  // so a page full of CTAs never exhausts the pool.
  ...
  const rootClassName = `specular-button specular-button--${size}${
    className ? ` ${className}` : ""
  }`;
  ...
  // CTAs on this site navigate, so render a real link when an href is given —
  // keeps middle-click, open-in-new-tab, and crawlable markup working.
  if (href !== undefined) {
